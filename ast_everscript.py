...
class Function(BaseBox):
    def __init__(self, name, script, args, function_args=[]):
        ...
        print(f"Function.init({name}, {args})")
...
class _Address(BaseBox):
    def __init__(self, value):
        self.value = value
        print(f"Address.init({value})")

    def eval(self):
        num = int(self.value.value, 16)
        print(f"Address.eval({num})")
        return num

class Word(_Function_Base):
    def __init__(self, value):
        self.value = value
        print(f"Word.init({value})")

    def eval(self):
        num = int(self.value.value, 16)
        print(f"Word.eval({num})")
        return num
...
class Label_Jump(BaseBox):
    def __init__(self, value):
        ...
        print(f"Label_Jump.init({self.value})")

    def eval(self):
        num = -1
        if self.value == "TEST":
            num = 1
        print(f"Label_Jump.eval({num})")
        return num

class Label_Destination(BaseBox):
    def __init__(self, value):
        self.value = re.sub(":", "", value.value)
        print(f"Label_Destination.init({self.value})")

    def eval(self):
        num = -1
        if self.value == "TEST:":
            num = 1
        print(f"Label_Destination.eval({num})")
        return num
...
class Call(_Function_Base):
    def __init__(self, function):
        ...
        print(f"Call.init({function}, {self.address})")
...
class End(_Function_Base):
    ...
    def eval(self):
        print(f"End.eval({self.label.eval()})")
        return 0
...
class Function_Transition(_Function_Base): # TODO
    ...
    def eval(self):
        print(f"Function_Transition.eval(map={self.map.eval()}, x={self.x.eval()}, y={self.y.eval()}, value={self.direction.eval()})")
        return 0
...
class Function_Eval(_Function_Base):
    def __init__(self, script):
        print(f"Function_Eval.init({script.value})")
        self.script = script

    def eval(self):
        print(f"Function_Eval.eval({self.script.value})")
        return 0
...
class Function_Goto(_Function_Base):
    def __init__(self, label):
        print(f"Function_Goto.init({label.value})")
        self.label = label
        self.distance = -1

    def eval(self):
        print(f"Function_Goto.eval({self.label.eval()})")
        return 0
...
class Enum(BaseBox):
    def __init__(self, name, values):
        print(f"Enum.init({name}, {values})")
        self.name = name.value
        self.values = values

    def eval(self):
        print(f"Enum.eval({self.values})")
        return 0
class Enum_Entry(BaseBox):
    def __init__(self, name, value):
        print(f"Enum_Entry.init({name}, {value})")
        self.name = name.value
        self.value = value

    def eval(self):
        print(f"Enum_Entry.eval({self.values})")
        return self.value.value

class Enum_Call(BaseBox):
    def __init__(self, generator, identifier):
        print(f"Enum_Call.init({identifier})")
    ...
    def eval(self):
        print(f"Enum_Call.eval({self.identifier})")
        return self.value
